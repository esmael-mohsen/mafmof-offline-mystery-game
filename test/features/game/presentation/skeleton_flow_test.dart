...
void main() {
  ...
  testWidgets('navigates from catalog to a started session', (tester) async {
    ...
    final playerNameFields = find.byType(TextField);
    expect(playerNameFields, findsNWidgets(6));
    await tester.enterText(playerNameFields.at(1), 'Ahmed');
    await tester.enterText(playerNameFields.at(2), 'Sara');
    await tester.enterText(playerNameFields.at(3), 'Laila');
    await tester.enterText(playerNameFields.at(4), 'Karim');
    await tester.enterText(playerNameFields.at(5), 'Noha');
    await tester.pump(const Duration(milliseconds: 500));

    final startSessionButton = find.text(AppText.startSessionAction);
    await tester.tap(startSessionButton);
    await _pumpUntilFound(tester, find.text(AppText.roleRevealTitle));

    expect(find.text(AppText.roleRevealTitle), findsOneWidget);
    expect(find.text(AppText.revealRoleAction), findsOneWidget);
  });
}
